...
@section('content')
    <div id="content" class="px-6 pb-8 pt-20 md:pt-16 w-full max-w-lg mx-auto mt-4">
        ...
        <form action="/admin/product/{{ $product->slug }}" method="POST" enctype="multipart/form-data">
            ...
            <input name="name" value="{{ old('name', $product->name) }}" class="sm:w-full md:w-1/2 p-2 mb-4">

            <label class="block text-grey-darker text-sm font-bold mb-2" for="body">
                Beskrivelse:
            </label>
            <textarea name="body" class="h-24 sm:w-full md:w-1/2 p-2 mb-4">{{ old('name', $product->body) }}</textarea>
            ...
            <input name="is_active" class="mr-2" @if ($product->is_active)value="is_active" @endif type="checkbox" @if ($product->is_active)checked="true" @endif>

            <label class="block text-grey-darker text-sm font-bold mb-2" for="is_featured">
                Profilert:
            </label>
            <input name="is_featured" @if ($product->is_featured)value="is_featured" @endif class="mr-2" type="checkbox" @if ($product->is_featured)checked="true" @endif>
